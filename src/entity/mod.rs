...
use crate::component::{Movable, Name, Named, Holding, Exit};
use crate::lib::get_sprite;

// TODO: Better name
fn make_named_entity(world: &mut World, (x, y): (u8, u8), name: Name, sprite: SpriteRender) -> Entity {
...
pub(crate) fn make_horizontal(world: &mut World, sprite_sheet_handle: Handle<SpriteSheet>) -> Entity {
...
pub(crate) fn make_walls(world: &mut World, sprite_sheet_handle: Handle<SpriteSheet>) -> Vec<Entity> {
    let sprite = get_sprite(sprite_sheet_handle, 10);
    let max = 10;
    let min = 1;
    let mut entities: Vec<Entity> = Vec::new();
    for x in min ..= max {
        entities.push(make_named_entity(world, (min, x), Name::Wall, sprite.clone()));
        entities.push(make_named_entity(world, (x, min), Name::Wall, sprite.clone()));
        entities.push(make_named_entity(world, (max, x), Name::Wall, sprite.clone()));
        entities.push(make_named_entity(world, (x, max), Name::Wall, sprite.clone()));
    }
    entities
}

pub(crate) fn make_floor(world: &mut World, sprite_sheet_handle: Handle<SpriteSheet>) -> Vec<Entity> {
...
pub(crate) fn make_crates(world: &mut World, sprite_sheet_handle: Handle<SpriteSheet>) -> Vec<Entity> {
    let sprite = get_sprite(sprite_sheet_handle, 6);
    let mut entities: Vec<Entity> = Vec::new();
    entities.push(make_named_entity(world, (2,3), Name::Crate, sprite.clone()));
    entities
}
