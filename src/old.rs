...
use amethyst::{
    ...
    derive::SystemDesc,
    ecs::{Join, DispatcherBuilder, Read, System, SystemData, World, WriteStorage, Write, prelude::{Component, DenseVecStorage}},
...
impl SimpleState for GameState {
    fn on_start(&mut self, data: StateData<'_, GameData<'_, '_>>) {
        ...
        println!(";;; Creating Movable component");
        let movable = Movable::new(reader);
        ...
        println!(";;; Created movable entity")
    }
}
...
impl<'a> System<'a> for MovementSystem {
    ...
    /// ...
    ///
    fn run(&mut self, (input_event_channel, mut movables): Self::SystemData) {
        for movable in (&mut movables).join() {
            for event in input_event_channel.read(&mut movable.reader) {
                match event {
